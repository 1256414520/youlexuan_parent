...
public class AliPayController {
    ...
    public Map createNative(){
       ...
       /* long out_trade_no = idWorker.nextId();*/
        ...
        //调用秒杀订单服务，根据用户id，获取对应秒杀订单信息
        TbSeckillOrder seckillOrder = seckillOrderService.searchOrderFromRedisByUserId(userId);
        if(seckillOrder!=null){

            BigDecimal bigDecimalMoney_Yuan = seckillOrder.getMoney();
            BigDecimal CS = new BigDecimal(100);
            BigDecimal bigDecimalMoney_Fen = bigDecimalMoney_Yuan.multiply(CS);
            System.out.println("预下单金额:"+bigDecimalMoney_Fen.longValue());

            Map map = aliPayService.createNative(seckillOrder.getId()+"",  bigDecimalMoney_Fen.longValue()+ "");
            return map;
        }
        ...
        while (true){
           //调用查询指定订单编号的订单状态方法
            ...
            if(map.get("tradestatus")!=null&&map.get("tradestatus").equals("TRADE_SUCCESS")){
                ...
                //调用秒杀订单服务，把秒杀订单从redis保存到数据库
              seckillOrderService.saveOrderFromRedisToDb(userId,Long.parseLong(out_trade_no),(String) map.get("trade_no"));
                break;
            }
            //2.2、第二种情况 ，交易关闭，交易结束
